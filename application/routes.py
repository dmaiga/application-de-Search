...
def profiles_user(app,db,bcrypt):
    ...
    def signup():
        ...
        Le premier utilisateur inscrit est automatiquement un admin.
        """
        if request.method == 'POST':
            # Récupérer les données du formulaire
            first_name = request.form.get('first_name')
            ...
            if not is_email_valid(email):  # Utilisation de la fonction is_email_valid importée depuis utils
                flash("L'adresse email n'est pas valide.", 'error')
                return redirect(url_for('auth'))
            ...
            # Déterminer si c'est le premier utilisateur
            is_first_user = User.query.count() == 0

            # Définir le rôle en fonction de l'ordre d'inscription
            ...
            user = User(
                ...
                role=role  # Rôle défini en fonction de l'ordre d'inscription
            )
